...
(function($)
{
	...
	Redactor.fn = $.Redactor.prototype = {
		...
		image: function()
		{
          ...
          // HACK
					var $figure = $('<figure>')
            .addClass("obj_container").addClass("image").attr("data-src", json.url)
		...
		line: function()
		{
          ...
          // HACK - from v.1.1.0
					...
					{
            // HACK
            if($tmpPrev.prop('tagName') === 'FIGURE') {
              $tmpPrev.remove();
              this.selection.restore();
            } else {
              e.preventDefault();

              $tmpPrev.append(this.marker.get()).append(tmpBlock.innerHTML);
              $tmpBlock.remove();
              this.selection.restore();
            }
...
//					}
		...
		observe: function()
		{
          ...
          // HACK
					this.core.editor().find('a:not(".embed")').on('touchstart.redactor.' + this.uuid + ' click.redactor.' + this.uuid, $.proxy(this.observe.showTooltip, this));
